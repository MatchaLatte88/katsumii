<template>
  <section id="features" class="section-band px-5 py-20 sm:px-6 sm:py-24 lg:px-10">
    <div class="mx-auto max-w-7xl">
      ...
      <div class="reveal mt-8 overflow-hidden rounded-full border py-2" :class="isDark ? 'border-blue-400/20 bg-slate-900' : 'border-gray-200 bg-white/75'">
        <div class="feature-marquee flex min-w-max gap-2 px-2">
          <span
            ...
            :class="[
              ...
              isDark ? 'border-cyan-300/30 bg-cyan-400/10 text-cyan-200' : 'border-teal-200 bg-teal-50 text-teal-700'
            ]"
          >
            {{ item }}
          </span>
      ...
      <div
        ...
        :class="isDark ? 'border-cyan-300/20 bg-cyan-400/10 text-slate-300' : 'border-teal-200/80 bg-teal-50/75 text-gray-700 shadow-sm'"
      >
        <InformationCircleIcon class="mt-0.5 h-5 w-5 flex-none sm:mt-0" :class="isDark ? 'text-cyan-200' : 'text-teal-600'" />
      ...
      <article
        ...
        :class="isDark ? 'border-cyan-300/18 bg-slate-900/72 shadow-[0_28px_90px_-54px_rgba(0,0,0,0.82)]' : 'border-gray-200/85 bg-white/82 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.38)]'"
      >
        <div class="relative z-10">
          <p :class="['text-xs font-semibold uppercase tracking-[0.18em]', isDark ? 'text-cyan-200' : 'text-teal-700']">
            {{ t('features.spotlight.label') }}
          </p>
          <h3 :class="['mt-3 font-display text-3xl font-semibold tracking-tight sm:text-4xl', isDark ? 'text-slate-100' : 'text-gray-900']">
          ...
          <ul class="mt-6 space-y-3">
            <li
              ...
              :class="isDark ? 'border-blue-400/20 bg-slate-900' : 'border-gray-200 bg-gray-50/80'"
            >
              <CheckIcon class="mt-0.5 h-5 w-5 flex-none" :class="isDark ? 'text-cyan-200' : 'text-teal-600'" />
              <span :class="['text-sm', isDark ? 'text-slate-200' : 'text-gray-700']">{{ point }}</span>
            </li>
          </ul>
        </div>
          ...
          <div class="relative w-full">
            ...
            <div class="mt-4 grid grid-cols-3 gap-3">
              <div
                ...
                :class="isDark ? 'border-blue-400/25 bg-slate-950/75' : 'border-gray-200 bg-white/95'"
              >
                <p :class="['text-base font-semibold', isDark ? 'text-slate-100' : 'text-gray-900']">{{ kpi.value }}</p>
                <p :class="['text-[11px]', isDark ? 'text-slate-400' : 'text-gray-500']">{{ kpi.label }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <article
          ...
          :class="isDark ? 'border-cyan-300/14 bg-slate-900/62 hover:border-cyan-300/28 hover:bg-slate-900/82' : 'border-gray-200/85 bg-white/78 hover:border-teal-200 hover:shadow-[0_18px_55px_-35px_rgba(15,23,42,0.34)]'"
        >
          <component
            :is="feature.icon"
            class="h-6 w-6"
          ...
          />
          <h3 :class="['mt-4 text-lg font-semibold', isDark ? 'text-slate-100' : 'text-gray-900']">{{ feature.name }}</h3>
          <p :class="['mt-3 text-sm leading-relaxed', isDark ? 'text-slate-300' : 'text-gray-600']">{{ feature.description }}</p>
        </article>
      </div>
    </div>
  </section>
